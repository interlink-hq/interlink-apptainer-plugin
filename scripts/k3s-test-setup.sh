#!/bin/bash
# k3s-test-setup.sh - Set up ephemeral K3s cluster for Apptainer plugin integration testing
#
...
echo "=== Setting up interLink Apptainer plugin integration test environment ==="
...
# ---------------------------------------------------------------------------
# Build Apptainer plugin Docker image from source
# ---------------------------------------------------------------------------
echo ""
echo "=== Building Apptainer plugin Docker image ==="
docker build -f "${PROJECT_ROOT}/docker/Dockerfile" \
  -t interlink-apptainer-plugin:local "${PROJECT_ROOT}" \
  2>&1 | tee "${TEST_DIR}/build-plugin.log"
echo "✓ Apptainer plugin image built"
...
# ---------------------------------------------------------------------------
# Start Apptainer plugin container
# ---------------------------------------------------------------------------
echo ""
echo "=== Starting Apptainer plugin container ==="
docker run -d --name interlink-plugin \
  --network interlink-net \
  -p 4000:4000 \
  --privileged \
  -v "${TEST_DIR}/plugin-config.yaml:/etc/interlink/ApptainerConfig.yaml:ro" \
  -e SHARED_FS=true \
  -e APPTAINERCONFIGPATH=/etc/interlink/ApptainerConfig.yaml \
  interlink-apptainer-plugin:local

sleep 3
if ! docker ps --filter "name=interlink-plugin" --filter "status=running" | grep -q interlink-plugin; then
  echo "ERROR: Apptainer plugin container failed to start"
  docker logs interlink-plugin 2>&1
  exit 1
fi
echo "✓ Apptainer plugin container started"

# ---------------------------------------------------------------------------
# Smoke-test: verify Apptainer works inside the plugin container
# ---------------------------------------------------------------------------
echo ""
echo "=== Running Apptainer smoke test ==="
set +e
SMOKE_OUTPUT=$(docker exec interlink-plugin apptainer exec docker://alpine:3.20 echo "Apptainer smoke test passed" 2>&1)
SMOKE_STATUS=$?
set -e

if [ "${SMOKE_STATUS}" -ne 0 ]; then
  echo "  WARNING: Apptainer smoke test failed (exit code ${SMOKE_STATUS}):"
  echo "${SMOKE_OUTPUT}"
  echo "  The plugin may still work; continuing..."
else
...
# ---------------------------------------------------------------------------
# Wait for, approve, and verify kubelet-serving CSRs
# ---------------------------------------------------------------------------
echo ""
...
echo ""
echo "=== interLink Apptainer plugin e2e test environment is ready ==="
