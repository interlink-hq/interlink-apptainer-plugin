#!/bin/bash
# k3s-test-run.sh - Run integration tests on K3s cluster with Apptainer plugin
#
...
echo "=== Running interLink Apptainer plugin integration tests ==="
...
# Create test configuration with Apptainer-specific settings.
...
values:
  ...
  annotations:
    apptainer-job.vk.io/image-root: "docker://"
...
# Run pytest excluding tests not applicable to the Apptainer plugin.
# - rclone: not supported (no SLURM mount provider)
# - limits: resource limits not enforced by Apptainer
# - stress: excluded for basic CI
# - multi-init: excluded for stability
# - fail: excluded (test intended to fail)
source .venv/bin/activate
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml
