...
// StopHandler kills the running job process, updates JIDs and cleans up files.
func (h *SidecarHandler) StopHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.G(h.Ctx).Info(sessionContextMessage, "Apptainer Sidecar: received Stop call")
	...
	if statusCode != http.StatusOK {
		w.Write([]byte("Some errors occurred deleting containers. Check Apptainer Sidecar's logs"))
	} else {
