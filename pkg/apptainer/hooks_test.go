...
// testApptainerConfig returns a minimal ApptainerConfig suitable for unit-testing
// hook and probe generation functions.  It uses a recognisable apptainer path so
// tests can assert that exec hooks are dispatched via the container runtime.
func testApptainerConfig() ApptainerConfig {
	return ApptainerConfig{
		ApptainerPath:           "/usr/bin/apptainer",
		ApptainerDefaultOptions: []string{"--nv"},
		ImagePrefix:             "docker://",
	}
}
...
func TestGeneratePreStopTrap_NoHooks(t *testing.T) {
	...
	if got := generatePreStopTrap(testApptainerConfig(), commands); got != "" {
...
func TestGeneratePreStopTrap_ExecHook_WithRuntime(t *testing.T) {
	...
	got := generatePreStopTrap(testApptainerConfig(), commands)
	if got == "" {
	...
	// Exec hook must be dispatched via apptainer exec.
	if !strings.Contains(got, "/usr/bin/apptainer") {
		t.Error("expected apptainer path in exec hook invocation")
	}
	if !strings.Contains(got, "exec") {
		t.Error("expected 'exec' subcommand in apptainer invocation")
	}
...
func TestGeneratePreStopTrap_ExecHook_NoRuntime(t *testing.T) {
	// When ApptainerPath is empty the exec hook falls back to host execution.
	cfg := ApptainerConfig{} // ApptainerPath is empty
	commands := []ContainerCommand{
		{
			containerName:   "app",
			isInitContainer: false,
	...
	// Must NOT contain an apptainer invocation in the fallback path.
	if strings.Contains(got, "apptainer") {
		t.Error("fallback exec hook should not contain 'apptainer'")
	}
}
...
func TestGeneratePostStartScript_NoHook(t *testing.T) {
	...
	if got := generatePostStartScript(testApptainerConfig(), cmd); got != "" {
...
func TestGeneratePostStartScript_ExecHook_WithRuntime(t *testing.T) {
	...
	got := generatePostStartScript(testApptainerConfig(), cmd)
	if got == "" {
		t.Fatal("generatePostStartScript returned empty string, expected script fragment")
	}
	// Must use apptainer exec (coherent with executeExecProbe).
	if !strings.Contains(got, "/usr/bin/apptainer") {
		t.Error("expected apptainer path in exec hook invocation")
	}
	if !strings.Contains(got, "timeout") {
...
func TestGeneratePostStartScript_ExecHook_NoRuntime(t *testing.T) {
	...
	if strings.Contains(got, "apptainer") {
		t.Error("fallback should not contain 'apptainer'")
	}
}
...
// ---------------------------------------------------------------------------
// generatePostStartScript — /tmp bind-mount injection
// ---------------------------------------------------------------------------
